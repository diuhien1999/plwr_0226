...
test("GET all bookings 111", async ({request}) => {
   const response = await request.get('https://restful-booker.herokuapp.com/booking');
   expect(response.status()).toBe(200);
   console.log(await response.json());
...
// using zod to validate the response schema
...
//   const patchResponse = await request.put('https://restful-booker.herokuapp.com/booking/1', {
//     headers: {
...
//   expect(patchData.totalprice).toBe(250);
//   expect(patchData.additionalneeds).toBe("Dinner");
// });
